...
angular.module('BeehivePortal')
  .controller('ThingDetailController', ['$scope', '$rootScope', '$state', 'AppUtils', '$$Thing', '$$Type', '$uibModal', '$timeout', '$$Trigger', '$log', function($scope, $rootScope, $state, AppUtils, $$Thing, $$Type, $uibModal, $timeout, $$Trigger, $log) {
    ...
    $scope.init = function(){
        ...
        searchThingCommands(globalThingID, $scope.startDate.getTime(), $scope.endDate.getTime() + 24 * 60 * 60 * 1000 - 1);
    };
    ...
    function searchThingCommands(thingID, startDate, endDate){
        var params = {
            globalThingID: $state.params.thingid,
            start: startDate,
            end: endDate
        };
